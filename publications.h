...
#define MAX_YEAR (CURRENT_YEAR + 1)
#define MAX_DIST (1000)

/**
 * Will contain all the data that is necessary in order to implement the API
 * given below.
 * Define its fields in publications.c or any other file so that said fields
 ...
 *                      if it is 0, then it is considered that there is no limit
...
/**
 * Computes the number of authors that have published papers for an institution
 * under a specific field of study.
 *
 * @param data          the data structure implemented by you
 ...
 * Each bucket of the histogram represents a year (in increasing order) since
 ...
 * possible paper written by the one given.
 * The best author is the one with the maximum score according to the following
 * formula:
 * score_i = e^{-erdos(author, author_i)} *
 * \sum_{j=1}^{num_papers_author_i}{num_citations_paper_j * impact_factor_j}
 *
 * @param data          the data structure implemented by you
 * @param id_authort    he id of the author looking for a coordinator
